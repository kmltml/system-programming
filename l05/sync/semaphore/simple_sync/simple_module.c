...
ssize_t simple_read(struct file *filp, char __user *user_buf, size_t count,
		    loff_t *f_pos)
{
	char *local_buf = NULL;
	int length_to_copy;
	...
	if (down_interruptible(&msg_pos_sem) != 0) {
		err = -EINTR;
		goto cleanup;
	}
