...
unsigned long buffer_lock_flags;
	...
	size_t count, loff_t *f_pos)
{
	...
	list_for_each_entry(data, &buffer, list) {
		...
		// We are in the correct entry
                ...
                memcpy(temp_buffer + copied, data->contents, to_copy);
	...
	*f_pos += real_length;
        ...
        if (copy_to_user(user_buf, temp_buffer, copied)) {
	...
	size_t count, loff_t *f_pos)
{
	...
	for (i = 0; i < count; i += INTERNAL_SIZE) {
		...
		list_add_tail(&(data->list), &temp_list);
	...
	write_count++;
        ...
        list_splice_tail(&temp_list, &buffer);
