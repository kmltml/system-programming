...
typedef struct {
  ...
  pthread_cond_t reader_enter;
  int reader_count;
  int max_reader_count;
} shared_obj;
...
int reader(void *data) {
  ...
  for (int i = 0; i < READER_TURNS; i++) {
    ...
    while (obj->reader_count >= obj->max_reader_count) {
      pthread_cond_wait(&obj->reader_enter, &obj->reader_count_mx);
    }
    int readers = ++obj->reader_count;
    ...
    // Read
    printf("(R) Reader %d started reading from %d (%d/%d)...\n", threadId, obj_index, readers, obj->max_reader_count);
    // Read, read, read
    ...
    obj->reader_count--;
    pthread_cond_broadcast(&obj->reader_enter);
...
int main(int argc, char *argv[]) {
  ...
  for(size_t i = 0; i < OBJ_COUNT; i++) {
    objects[i].max_reader_count = 1 + i;
  }
